...
async def send_move_orders(websocket):
    ...
    Units prioritize attacking visible enemy units when they are stronger, then exploring unknown parts of the map
    and retreating if needed. Different unit types have varying priorities, and units consider terrain movement costs
    and benefits such as hills for vision, mountains as barriers. The function applies a simple A* pathfinding
    algorithm for movement towards specific targets. Units learn from previous game data and adapt their strategies.
    """
...
def determine_target(unit, enemies, enemy_threat):
    ...
    # If there are visible enemies and our unit's health is at least 20, attack the nearest enemy.
    elif enemies and health > 20:
        return astar((current_r, current_c), (enemies[0].get('row'), enemies[0].get('col')))
    # If our unit's health is less than 20, retreat.
    elif health <= 20:
        return retreat(current_r, current_c)
    # If there are no visible enemies or our unit's health is less than 20, explore the map.
    else:
