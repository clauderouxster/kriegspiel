    ...
    while not game_over:
        if current_units and game_map:
            ...
            for unit_to_move in red_units:
                ...
                currentMovePoints = unit_to_move.get('currentMovePoints')
                enemies = [u for u in VISIBLE_UNITS if u.get('armyColor') != player_army_color]
                enemy_threat = sum(enemy['health'] for enemy in enemies)

                target_r, target_c = determine_target(unit_to_move, enemies, enemy_threat)
...
def determine_target(unit, enemies, enemy_threat):
    ...
    if enemies and enemy_threat > health:
        # If the enemy threat is greater than our unit's health, retreat.
